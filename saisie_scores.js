...
function bindNavigation() {
  const [prevBtn, finishBtn, nextBtn] = document.querySelectorAll('.tool-box-buttons');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // 2. Gestion du mode édition historique et du clic sur “Retour”
  toggleHeaderLink();
  const backLink = document.querySelector('.back-link');
  if (backLink) {
    backLink.addEventListener('click', e => {
      // Si aucune modification n’a été faite, on nettoie simplement et on revient
      if (!isModified) {
        localStorage.removeItem('currentGame');
        localStorage.removeItem('scores');
        localStorage.removeItem('currentTurn');
        localStorage.removeItem('historyEditMode');
        localStorage.removeItem('editHistoryIndex');
        return; // Laisse l’ancre faire la navigation vers historique.html
      }

      // Sinon, une modification a eu lieu : on affiche un confirm
      e.preventDefault(); // Empêche la navigation automatique
      const userChoice = window.confirm(
        "Vous avez modifié des scores. Cliquez sur \"OK\" pour enregistrer vos modifications,\noun sur \"Annuler\" pour annuler et revenir sans enregistrer."
      );
      ...
      if (userChoice) {
        ...
        const editIdx = parseInt(localStorage.getItem('editHistoryIndex'), 10);
        if (!isNaN(editIdx)) {
          // Remplacer l’entrée existante dans l’historique
          hist[editIdx] = {
            date: new Date().toISOString(),
            players: JSON.parse(localStorage.getItem('currentGame')).players,
            scores
      ...
      // Dans tous les cas, on retourne à l’historique et on nettoie les clés
      localStorage.removeItem('currentGame');
      localStorage.removeItem('scores');
      localStorage.removeItem('currentTurn');
      localStorage.removeItem('historyEditMode');
