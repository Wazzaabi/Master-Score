document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('incr-player').addEventListener('click', () => {
      if (nbr < maxPlayers) { nbr++; nbrDisplay.textContent = nbr; updateNameFields(); }
    });
    document.getElementById('decr-player').addEventListener('click', () => {
      if (nbr > minPlayers) { nbr--; nbrDisplay.textContent = nbr; updateNameFields(); }
    });
  
    document.getElementById('start-game').addEventListener('click', () => {
      const names = Array.from(namesContainer.querySelectorAll('input'))
                         .map(i => i.value.trim());
      if (names.some(n => !n)) {
